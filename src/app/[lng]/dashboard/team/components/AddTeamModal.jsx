...
export default function AddTeamModal({ isOpen, onClose, onAdd, lng }) {
  ...
  const handleImageUpload = async () => {
    ...
    try {
      ...
      const fileName = `team/${Date.now()}_${Math.random().toString(36).substring(7)}.${fileExt}`
  ...
  const onSubmit = async (data) => {
    ...
    try {
      ...
      toast.error(
        lng === 'ar' 
          ? 'فشل إضافة عضو الفريق. يرجى المحاولة مرة أخرى.' 
      ...
      >
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="p-6 space-y-6">
          ...
          <div className="flex items-center justify-end gap-3 pt-4 border-t border-gray-200">
            ...
            >
              {uploading ? 'Uploading Photo...' : loading ? 'Adding...' : 'Add Team Member'}
            </button>
          </div>
        </form>
      </div>
