...
export default function EditPlanModal({ plan, isOpen, onClose, onSave, lng }) {
  ...
  useEffect(() => {
    if (plan && isOpen) {
      setValue('titleEn', plan.title?.en || '')
      setValue('titleAr', plan.title?.ar || '')
      setValue('type', plan.type || 'basic')
      setValue('price', plan.price || 0)
      setValue('discount', plan.discount || 0)
      setValue('currencyEn', plan.currency?.en || 'AED')
      setValue('currencyAr', plan.currency?.ar || 'درهم')
      setValue('display_order', plan.display_order || 0)
      
      setShowContactSales(plan.show_contact_sales || false)
      setFeaturesEn(plan.features?.en || [''])
      setFeaturesAr(plan.features?.ar || [''])
    }
  }, [plan, isOpen, setValue])
  ...
  const onSubmit = async (data) => {
    ...
    try {
      ...
      toast.error(
        lng === 'ar' 
          ? 'فشل تحديث الخطة. يرجى المحاولة مرة أخرى.' 
