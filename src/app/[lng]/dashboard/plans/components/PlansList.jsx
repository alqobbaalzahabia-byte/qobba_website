...
export default function PlansList({ plans, error, lng }) {
    ...
    try {
      const { data, error } = await supabase
        ...
        .insert([{
          ...
          show_contact_sales: formData.show_contact_sales,
  ...
  const handleSave = async (planId, formData) => {
    try {
      const { data, error } = await supabase
        .from('plans')
        .update({
  ...
  const handleDeleteConfirm = async () => {
    ...
    try {
      ...
      toast.error(
        lng === 'ar' 
          ? 'فشل حذف الخطة. يرجى المحاولة مرة أخرى.' 
      ...
      {plans?.length === 0 ? (
        <div className="text-center py-12 bg-gray-50 rounded-lg border-2 border-dashed border-gray-300">
          <div className="flex flex-col items-center gap-3">
            ...
            <p className="text-gray-500 text-lg font-medium">No plans found</p>
            <p className="text-gray-400 text-sm">Get started by adding your first pricing plan</p>
            <button
              onClick={() => setIsAddModalOpen(true)}
          ...
          >
            ...
            <div className="p-6">
              <p className="text-gray-700 mb-6">
                {lng === 'ar' 
                  ? `هل أنت متأكد من حذف "${planToDelete.title?.[lng] || planToDelete.title?.en || 'هذه الخطة'}"؟ لا يمكن التراجع عن هذا الإجراء.`
                  : `Are you sure you want to delete "${planToDelete.title?.[lng] || planToDelete.title?.en || 'this plan'}"? This action cannot be undone.`
                }
              </p>
