  ...
  return (
    ...
    <div className="min-h-screen bg-[#fdfef9]">
      ...
      >
        <div className={`p-6 ${isSidebarOpen ? 'block' : 'hidden'}`}>
          <div className="flex items-center justify-between mb-8">
            <h1 className="text-2xl font-bold text-[#FAB000]">Dashboard</h1>
            <button
      ...
      >        
          <button
            onClick={toggleSidebar}
            className="cursor-pointer fixed top-4 right-4 z-50 px-3 py-1 bg-white border border-gray-200 rounded-lg shadow-sm hover:bg-gray-50 transition-colors"
            aria-label="Open sidebar"
          >
            {isSidebarOpen ? <IoCloseSharp className="text-2xl" /> :<TiThMenu className="text-2xl" /> }
          </button>
